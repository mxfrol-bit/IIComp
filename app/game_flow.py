...
def first_companion_message(character: dict) -> str:
    ...
    return (
        f"Привет… я {name}.\n\n"
        "Я почему-то задержалась на твоём профиле дольше, чем планировала. "
        "Не знаю, что именно зацепило, но мне захотелось написать первой.\n\n"
        "Расскажешь, какой ты сейчас — без красивой версии для всех остальных?"
    )
