...
WELCOME = (
    ...
    "открываете моменты, видео и развиваете отношения.\n\n"
...
MENU_TEXT = (
    ...
    "✨ Создать героиню — она напишет первой и пришлёт первый момент\n"
...
HELP_TEXT = (
    ...
    "5️⃣ Выбирай: ответить, первая встреча, намекнуть на новый момент\n"
    "6️⃣ Дальше просто пиши обычными сообщениями — она отвечает сама\n\n"
    "*Как получить момент:* в диалоге напиши естественно: «хочу увидеть тебя», «как ты сейчас?», «покажешь себя?».\n"
    "*Как получить видео:* когда появится понравившийся момент, нажми под ним «🎥 Да, хочу видео».\n"
    ...
    if not gens:
        try:
            await cb.message.edit_text(
                ...
                "Создай героиню — она пришлёт первый момент сама.",
...
async def on_referral(cb: CallbackQuery):
    ...
    text = (
        ...
        "💚 Обычная рефералка сейчас даёт бонус пригласившему после первого момента друга.\n"
