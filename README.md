...
- **🔄 Ещё такой момент** — кнопка под фото для перегенерации с новым seed
...
- Фразы типа `хочу увидеть тебя`, `как ты сейчас?`, `покажешь себя?` запускают новый момент из диалога.
- `🎲 Ролевые сцены` — быстрый старт сценариев: свидание, прогулка, вечер дома, сюрприз.
- Под моментом появилась кнопка `🎥 Да, хочу видео` — персонаж как будто отправляет короткое видео из этого момента.
...
## Test referral link for 2000 credits

Run migration:

```sql
-- Supabase SQL Editor
-- see supabase/006_promo_referral.sql
```
...
```text
https://t.me/YOUR_BOT_USERNAME?start=beta2000
```

Each user can claim the test bonus once. Admins see this link in the “🤝 Друзья” section.

## Game-first flow update

Новая логика продукта:

1. Пользователь нажимает **✨ Создать героиню**.
2. Выбирает имя, возраст, волосы, телосложение, стиль.
3. Бот автоматически создаёт первый момент персонажа — это знакомство и аватар.
4. Героиня сама пишет первое сообщение.
5. Пользователь выбирает: ответить, первая встреча, намекнуть на новый момент.
6. Первая встреча создаёт отдельный момент и запускает живой диалог.
7. Дальше пользователь пишет обычными сообщениями, а бот отвечает от лица персонажа.
8. Романтика и soft 18+ открываются позже, через прогресс отношений и тарифный доступ.

Soft 18+ не показывается как ранний основной сценарий. Это поздний слой игры, а не первая кнопка.


## Free chat v4

В этой версии основной режим — свободный чат. Пользователь может писать героине обычным текстом, а кнопки под ответами работают как мягкие подсказки, а не как обязательное меню.

Что изменено:

- после открытия чата появляются подсказки: спросить о ней, предложить встречу, сделать комплимент;
- после каждого ответа Claude появляются новые подсказки, зависящие от шкалы близости;
- нажатие на подсказку обрабатывается как обычная реплика пользователя;
- шкала близости растёт даже от нормального свободного общения, а не только от заранее зашитых вариантов;
- Claude получил усиленный системный стиль: живой чат, постоянный мягкий флирт, вопросы и крючки для продолжения;
- технические слова вроде “генерация/промпт/модель” убраны из пользовательского флоу;
- просьбы “хочу увидеть тебя / как ты сейчас / покажешь момент” по-прежнему запускают отправку фото;
- просьбы про видео мягко ведут к кнопке видео под уже созданным моментом.

Новых миграций для v4 нет. Достаточно уже накатанных `005_companion_game.sql` и `006_promo_referral.sql`.
