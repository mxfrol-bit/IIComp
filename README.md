...
3. Бот автоматически генерирует первое фото персонажа — это игровая карточка/аватар.
4. Героиня сама пишет первое сообщение.
5. Пользователь выбирает: ответить, первая встреча, попросить фото.
6. Первая встреча генерирует отдельную сцену-фото и запускает ролевой диалог.
